...
%   Copyright 2018-2019 The MathWorks, Inc.
    ...
    if isa(varargin{1},'matlab.graphics.Graphics') && ...
            (isa(varargin{1},'matlab.ui.internal.mixin.CanvasHostMixin') || ...
    ...
    if ~isa(parent,'matlab.graphics.layout.Layout')
        layparent=matlab.graphics.internal.getCurrentLayout(parent);
        if ~isempty(layparent)
            parent=layparent;
        end
    end
...
function parent = findParentArg(nvpairs)
    ...
    if any(parentIndex)
        ...
        if isa(p,'matlab.graphics.Graphics') && ...
            (isa(p,'matlab.ui.internal.mixin.CanvasHostMixin') || ...
